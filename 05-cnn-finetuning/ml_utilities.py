...
def internal_load_unlabeled_dataset(filelist_name, db_folder_path):
    files = []
...
def load_unlabeled_dataset(filelist_name, db_folder_path, cache=None):
    if cache is not None:
        cached_function = cache.cache(internal_load_unlabeled_dataset)
    else:
        cached_function = internal_load_unlabeled_dataset
...
def tf_create_graph(class_num, tf, optimizer='momentum'):
    ...
    with tf.contrib.slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
        logits, end_points = mobilenet_v1.mobilenet_v1(X, depth_multiplier=depth_multiplier, num_classes=1001,
    ...
    anchor = end_points[
        'AvgPool_1a']  # end_point è un dictionary che ci permette di accedere per nome ai livelli del modello
    ...
    if optimizer == 'momentum':
        ...
        raise ValueError(
            'Optimizer non supportato. Per utilizzarlo è necessario aggiornare manualmente il file utils.py.')
...
def tf_compute_test_accuracy(test_x, test_y, minibatch_size, class_num, sess, accuracy, X, y, training, Y_proba):
    ...
    for iteration in range(n_iterations_per_epoch_test):
        ...
        minibatch_acc, minibatch_prob = sess.run([accuracy, Y_proba],
                                                 feed_dict={X: X_minibatch, y: y_minibatch, training: False})
